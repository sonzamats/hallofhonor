#!/usr/bin/env python3
"""
Purple Heart registry scraper.

Scrapes recipient data from https://www.purpleheartregistry.net and
writes structured records to a JSON file.

Output: data/purple_heart_recipients.json
"""
...
BASE_URL = "https://www.purpleheartregistry.net"
DATA_DIR = Path(__file__).resolve().parent.parent / "data"
OUTPUT_FILE = DATA_DIR / "purple_heart_recipients.json"
...
def discover_listing_pages(session: requests.Session) -> list[str]:
    """Return all listing page URLs from the registry."""
    ...
    # Look for alphabetical or paginated navigation
    for a in soup.select("nav a, .pagination a, a.page-link, ul.pager a"):
        href = a.get("href")
        if href:
    ...
    # Probe query-string pagination if nothing found
    if len(pages) == 1:
        page_num = 2
        while page_num <= 200:
            ...
            try:
                ...
                soup = BeautifulSoup(resp.text, "lxml")
                if not soup.select("a[href], table tbody tr, .recipient, .card"):
                    break
...
def extract_recipient_links(page_url: str, session: requests.Session) -> list[str]:
    ...
    for a in soup.select("a[href]"):
        href = a.get("href", "")
        if any(kw in href for kw in ["recipient", "profile", "veteran", "hero"]):
...
def scrape_recipient(url: str, session: requests.Session) -> dict | None:
    ...
    photo_url = ""
    img = soup.select_one("img.photo, img.profile, .recipient img, article img")
    if img:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Scrape Purple Heart recipients from purpleheartregistry.net"
    )
